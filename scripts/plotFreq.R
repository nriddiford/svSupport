...
getFreqs <- function(..., infile = 'variants_out.txt'){
  ...
  excluded_samples <- c("A373R1", "A373R7", "A512R17", "A373R11", "A785-A788R1", "A785-A788R11", "A785-A788R3", "A785-A788R5", "A785-A788R7", "A785-A788R9")
 
  af_data <- filter(af_data, !sample %in% excluded_samples)
  ...
  #                               af_data$sample)
  # af_data %>%
  #   mutate(sample_short = ifelse(grepl('R', sample),
  ...
  sample_af_data <- af_data %>%
    ...
    mutate(allele_freq = as.numeric(as.character(allele_freq))) %>% 
    group_by(sample, hit) %>%
    mutate(count = seq(n())) %>%
    mutate(gene = paste(sample, hit, count, sep = "_")) %>% 
    ungroup()
