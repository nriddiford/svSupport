...
headers = ['sample', 'event', 'type', 'bam', 'locus', 'purity', 'read_depth', 'length(Kb)', 'bp1_locus', 'bp2_locus', 'affected_genes', 'guess']
...
with open(svs_file, 'r') as variants:
	...
	for name in depth_files:
        ...
        if "cnv" in variant['source'].lower():
            ...
            out_line = [variant['sample'], variant['event'], variant['type'], bam_in, variant['position'], variant['purity'], 'NA', variant['length(Kb)'], variant['bp1_locus'], variant['bp2_locus'], variant['affected_genes'], 'T']
        else:
